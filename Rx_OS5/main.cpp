...
//Receives the transmitters' time at an interval defined by the print ticker
void Print_Sensors() {
  // this runs in the lower priority thread
    ...
    debug("Time on air : %d\r\n", ( int ) (Radio.TimeOnAir ( MODEM_LORA, 4 )));
    debug("RSSI : %d\r\n", ( int )RssiValue);
    ...
    //Converts the array of bytes (Transmission) to an integer.
    int test_time = buffToInteger(Buffer);
    
    debug("Received packet: %d\r\n", test_time);
    //Fills the buffer with zero to prepare for next transmission
    for (int k=0; k<18; k++){
        Buffer[k] = 0;
    }
}
...
{
    int sent_time = (int)(buffer[0] << 24 | buffer[1] << 16 | buffer[2] << 8 | buffer[3]);
    return sent_time;
}
